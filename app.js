...
(() => {
  ...
  function scoreHeadline(str) {
    ...
    if (!str.trim()) {
      return { score: 0, wc: 0, chars: 0, tips: [] };
    }
    ...
    const charTarget =
      chars >= 30 && chars <= 60 ? 100 : 100 - Math.abs(45 - chars) * 2.2;
    ...
    bigrams.forEach((c) => {
      if (c > 1) repetitionPenalty += (c - 1) * 8;
    });
  ...
  function load() {
    try {
      ...
      const saved = JSON.parse(
        localStorage.getItem("neon-headline-rater") || "{}"
      );
      if (a !== null || b !== null) {
  ...
  ["input", "change", "blur", "keyup"].forEach((ev) => {
    els.inA.addEventListener(ev, () => {
      updateSide("A");
      save();
    });
    els.inB.addEventListener(ev, () => {
      updateSide("B");
      save();
    });
  });
